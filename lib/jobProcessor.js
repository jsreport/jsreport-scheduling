...
module.exports = class JobProcessor {
  constructor ({ beforeProcessJobListeners, executionHandler, documentStore, logger, Request, TaskType, options }) {
    if (!options.taskPingTimeout) {
      options.taskPingTimeout = 3 * options.interval
    }
  ...
  async process (options = {}) {
    try {
      ...
      if (this.currentlyRunningTasks.length >= this.options.maxParallelJobs) {
        this.logger.info('The process queue is full, waiting')
        return
      }
      ...
      const recoveryPromise = Promise.all(
        tasks.map(task => {
          ...
          return this.processOne(task.schedule, task, true)
        })
      )
      ...
      const processPromise = Promise.all(schedules.map(s => this.processOne(s, null, false)))
  ...
  _pingRunningTasks () {
    ...
    if (ids.length) {
      this.logger.info('Ping running task ' + ids.length)
    }
  ...
  async processOne (schedule, task, recovering) {
    if (this.currentlyRunningTasks.length >= this.options.maxParallelJobs) {
      this.logger.info('The process queue is full, waiting')
      return
    }
    ...
    try {
      ...
      if (count === 1) {
        return this.execute(schedule, task, { recovering }, req)
      }
    } catch (e) {
  ...
  async execute (schedule, task, { executeNow, recovering }, req) {
    this.logger.info(`Processing schedule ${schedule.name} is starting${executeNow ? ' (manual)' : ''}.`)
    ...
    try {
      ...
      if (templates.length > 0) {
        ...
        this.logger.info(`Processing schedule ${schedule.name} succeeded.`)
      ...
      this.logger.warn('Schedule template not found, marking task as error')
      ...
      this.logger.warn(`Processing schedule ${schedule.name} failed with : ${e.stack}`)
