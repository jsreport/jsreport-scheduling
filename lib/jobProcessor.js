      ...
      if (misfiredSchedules.length) {
        ...
        await Promise.all(misfiredSchedules.map(async (sched) => {
          ...
          const count = await this.documentStore.collection('schedules').update(
            ...
            localReqWithoutModificationDateUpdate(this.Request({}))
          )
  ...
  async processOne (schedule, task) {
    ...
    try {
      const count = await this.documentStore.collection('schedules').update(
        ...
        { $set: { state: 'planning' } },
        localReqWithoutModificationDateUpdate(req)
      )
...
function localReqWithoutModificationDateUpdate (req) {
  if (req == null) {
    return req
  }
  ...
  // original req
  const localReq = Object.assign({}, req)
  localReq.context = Object.assign({}, req.context)
