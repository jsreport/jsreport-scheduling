...
class Scheduling {
  constructor (reporter, definition) {
    ...
    this.reporter.on('express-configure', (app) => {
      app.get('/api/scheduling/nextRun/:cron', (req, res) => {
        try {
          const cron = new CronTime(req.params.cron)
      ...
      app.post('/api/scheduling/runNow', (req, res) => {
        this.reporter.documentStore.collection('schedules').find({
          ...
          return this.reporter.scheduling.jobProcessor.execute(schedule[0], null, true)
        }).then(function () {
          res.status(200).end()
        }).catch(function (err) {
  ...
  _updateNextRun (entity) {
    ...
    const intervalMS = cron._getNextDateFrom(new Date(entity.nextRun.getTime() + 1000)).toDate().getTime() - entity.nextRun.getTime()
